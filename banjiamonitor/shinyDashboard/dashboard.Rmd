...
```{r}
renderPlotly({
  fig <- plot_ly(hourly_inc, x = ~hour, y = ~ZKN, name = 'ZKN', type = 'scatter', mode = 'lines+markers', line = list(width = 3)) 
...
```{r}
renderPlotly({
  fig <- plot_ly(daily_inc, x = ~hour, y = ~ZKN, name = 'ZKN', type = 'scatter', mode = 'lines+markers') 
  fig <- fig %>% add_trace(y = ~XKY, name = 'XKY', mode = 'lines+markers') 
  fig %>% layout(yaxis = list(title = "increase"), autosize=TRUE, height = "99%")
}
)
```   
...
```{r}
selectInput("user_input", "Select a weibo user", choices=avail_user, selected = avail_user[1], multiple = FALSE,
  selectize = TRUE, width = NULL, size = NULL)

avail_weibo <- reactive({
  user_input = input$user_input
  weibo.data %>% filter(user==user_input) %>% filter(as.Date(crawler_time, tz="Asia/Taipei") == today) %>% 
...
weibo_link <- reactive({
  ...
  weibo.data %>% filter(user==user_input) %>% filter(as.character(post_time)==post_time_input) %>% distinct(link) %>% pull()
})
